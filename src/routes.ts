...
router.get("/books", async (ctx) => {
  ...
  const filters = ctx.query.filters as Array<{ from?: string; to?: string; author?: string; name?: string }>;
  ...
  try {
    ...
    for (const filter of filters) {
      const query = {} as { price: { $gte?: number, $lte?: number }, author: string, name: string }
      ...
      const result = await collection.find(query).map(
        doc => {
          const book: Book = {
